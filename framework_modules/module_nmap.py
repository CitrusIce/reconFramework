...
outputdir = os.path.join(basedir, "output", "nmap")
...
class Nmap(Module):
    ...
    def nmap_scan(self, ip):
        ...
        # try:
        logging.info(self.__class__.__name__ + " scan target: " + ip)
        nm = nmap.PortScanner()
...
def output_to_brutespray(result_dict):
    data_list = []
    ...
    filename = time.strftime("%Y-%m-%d-%H-%M", time.localtime()) + "_brute_targets.json"
